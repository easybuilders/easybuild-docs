# SimpleBuild configuration options {: #basic_usage_help }

*(corresponds with output of* ``eb --help`` *)*

## Usage

``eb [options] simpleconfig [...]``

## Description

Builds software based on simpleconfig (or parse a directory).
Provide one or more simpleconfigs or directories, use -H or --help more information.
...
Option flag                              |Option description
-----------------------------------------|----------------------------------------------------------------------------------------------------------------
``--configfiles=CONFIGFILES``            |Parse (additional) configfiles (type comma-separated list; default: /home/example/.config/simplebuild/config.cfg)
...
``-M, --missing-modules``              |Print list of missing modules for dependencies of specified simpleconfigs (default: False)
...
``-r PATH[:PATH], --robot=PATH[:PATH]``|Enable dependency resolution, using simpleconfigs in specified paths (type pathsep-separated list)
``--robot-paths=PATH[:PATH]``          |Additional paths to consider by robot for simpleconfigs (--robot paths get priority) (type pathsep-separated list; default: /home/example/work/simplebuild-simpleconfigs/simplebuild/simpleconfigs)
...
``--buildpath=BUILDPATH``                                        |Temporary build path (default: /home/example/.local/simplebuild/build)
``--containerpath=CONTAINERPATH``                                |Location where container recipe & image will be stored (default: /home/example/.local/simplebuild/containers)
...
``--include-simpleblocks=INCLUDE-SIMPLEBLOCKS``                      |Location(s) of extra or customized simpleblocks (type comma-separated list)
...
``--installpath=INSTALLPATH``                                    |Install path for software and modules (default: /home/example/.local/simplebuild)
...
``--logfile-format=DIR,FORMAT``                                  |Directory name and format of the log file (type comma-separated tuple; default: simplebuild,simplebuild-%(name)s-%(version)s-%(date)s.%(time)s.log)
...
``--module-naming-scheme=MODULE-NAMING-SCHEME``                  |Module naming scheme to use (default: SimpleBuildMNS)
...
``--package-naming-scheme=PACKAGE-NAMING-SCHEME``                |Packaging naming scheme choice (type choice; default: SimpleBuildPNS) (choices: SimpleBuildDebFriendlyPNS, SimpleBuildPNS)
``--packagepath=PACKAGEPATH``                                    |The destination path for the packages built by package-tool (default: /home/example/.local/simplebuild/packages)
``--prefix=PREFIX``                                              |Change prefix for buildpath, installpath, sourcepath and repositorypath (used prefix for defaults /home/example/.local/simplebuild)
...
``--repositorypath=REPOSITORYPATH``                              |Repository path, used by repository (is passed as list of arguments to create the repository instance). For more info, use --avail-repositories. (type comma-separated list; default: /home/example/.local/simplebuild/ebfiles_repo)
``--sourcepath=SOURCEPATH``                                      |Path(s) to where sources should be downloaded (string, colon-separated) (default: /home/example/.local/simplebuild/sources)
...
``--container-image-name=IMAGE-NAME``          |Custom name for container image (defaults to name of simpleconfig)
...
## Options for Simpleconfigs

Option flag                                            |Option description
-------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
``--create-index=CREATE-INDEX``                        |Create index for files in specified directory
``--fix-deprecated-simpleconfigs``                       |Fix use of deprecated functionality in specified simpleconfig files. (default: False)
...
``--inject-checksums=INJECT-CHECKSUMS``                |Inject checksums of specified type for sources/patches into simpleconfig file(s) (type choice; default: sha256) (choices: adler32, crc32, md5, sha1, sha256, sha512, size)
...
``--check-contrib``                                    |Runs checks to see whether the given simpleconfigs are ready to be contributed back (default: False)
``--check-github``                                     |Check status of GitHub integration, and report back (default: False)
``--check-style``                                      |Run a style check on the given simpleconfigs (default: False)
``--cleanup-simpleconfigs``                              |Clean up simpleconfig files for pull request (default: True; disable with --disable-cleanup-simpleconfigs)
...
``--from-pr=PR#``                                      |Obtain simpleconfigs from specified PR (type comma-separated list)
``--git-working-dirs-path=GIT-WORKING-DIRS-PATH``      |Path to Git working directories for SimpleBuild repositories (type <class 'str'>)
``--github-org=GITHUB-ORG``                            |GitHub organization (type <class 'str'>)
``--github-user=GITHUB-USER``                          |GitHub username (type <class 'str'>)
``--include-simpleblocks-from-pr=PR#``                   |Include simpleblocks from specified PR (type comma-separated list)
...
``--pr-target-account=PR-TARGET-ACCOUNT``              |Target account for new PRs (type <class 'str'>; default: simplebuilders)
...
``--review-pr-filter=REVIEW-PR-FILTER``                |Regex used to filter out simpleconfigs to diff against in --review-pr
``--review-pr-max=REVIEW-PR-MAX``                      |Maximum number of simpleconfigs to diff against in --review-pr (type <class 'int'>)
...
``--avail-simpleconfig-constants``                     |Show all constants that can be used in simpleconfigs (default: False)
``--avail-simpleconfig-licenses``                      |Show all license constants that can be used in simpleconfigs (default: False)
``-a, --avail-simpleconfig-params``                    |Show all simpleconfig parameters (include simpleblock-specific ones by using -e) (default: False)
``--avail-simpleconfig-templates``                     |Show all template names and template constants that can be used in simpleconfigs. (default: False)
...
``--check-eb-deps``                                  |Check presence and version of (required and optional) SimpleBuild dependencies (default: False)
...
``--simplestack=SIMPLESTACK``                            |Path to simplestack file in YAML format, specifying details of a software stack
``--last-log``                                       |Print location to SimpleBuild log file of last (failed) session (default: False)
``--list-simpleblocks=LIST-SIMPLEBLOCKS``                |Show list of available simpleblocks (type choice; default: simple) (choices: simple, detailed)
...
``--search=REGEX``                                   |Search for simpleconfig files in the robot search path, print full paths
``--search-filename=REGEX``                          |Search for simpleconfig files in the robot search path, print only filenames
``-S REGEX, --search-short=REGEX``                   |Search for simpleconfig files in the robot search path, print short paths
``--show-config``                                    |Show current SimpleBuild configuration (only non-default + selected settings) (default: False)
...
``--show-ec``                                        |Show contents of specified simpleconfig(s) (default: False)
``--show-full-config``                               |Show current SimpleBuild configuration (all settings) (default: False)
``--show-system-info``                               |Show system information relevant to SimpleBuild (default: False)
...
``--job-eb-cmd=EB-CMD``                    |SimpleBuild command to use in jobs (type str; default: eb)
...
``--job-output-dir=OUTPUT-DIR``            |Output directory for jobs (default: current directory) (default: /Volumes/work/simplebuild-docs/docs/version-specific)
...
``--allow-loaded-modules=ALLOW-LOADED-MODULES``                          |List of software names for which to allow loaded modules in initial environment (type comma-separated list; default: SimpleBuild)
``--allow-modules-tool-mismatch``                                        |Allow mismatch of modules tool and definition of 'module' function (default: False)
``--allow-use-as-root-and-accept-consequences``                          |Allow using of SimpleBuild as root (NOT RECOMMENDED!) (default: False)
...
``--checksum-priority=CHECKSUM-PRIORITY``                                |When checksums are found in both the SimpleConfig and the checksums.json fileDefine which one to use.  (type choice; default: simpleconfig) (choices: json, simpleconfig)
...
``--consider-archived-simpleconfigs``                                      |Also consider archived simpleconfigs (default: False)
``-C, --containerize``                                                   |Generate container recipe/image (default: False)
``--copy-ec``                                                            |Copy specified simpleconfig(s) to specified location (default: False)
``--cuda-cache-dir=PATH``                                                |Path to CUDA cache dir to use if enabled. Defaults to a path inside the build dir. (type <class 'str'>)
``--cuda-cache-maxsize=CUDA-CACHE-MAXSIZE``                              |Maximum size of the CUDA cache (in MiB) used for JIT compilation of PTX code. Leave value empty to let SimpleBuild choose a value or '0' to disable the cache (type <class 'int'>)
...
``--detect-loaded-modules=DETECT-LOADED-MODULES``                        |Detect loaded SimpleBuild-generated modules, act accordingly; supported values: error, ignore, purge, unload, warn (default: warn)
...
``--dump-autopep8``                                                      |Reformat simpleconfigs using autopep8 when dumping them (default: False)
``-e CLASS, --simpleblock=CLASS``                                          |simpleblock to use for processing the spec file or dumping the options
...
``--filter-deps=FILTER-DEPS``                                            |List of dependencies that you do *not* want to install with SimpleBuild, because equivalent OS packages are installed. (e.g. --filter-deps=zlib,ncurses) (type comma-separated list)
``--filter-ecs=FILTER-ECS``                                              |List of simpleconfigs (given as glob patterns) to *ignore* when given on command line or auto-selected when building with --from-pr. (e.g. --filter-ecs=*intel*) (type comma-separated list)
``--filter-env-vars=FILTER-ENV-VARS``                                    |List of names of environment variables that should *not* be defined/updated by module files generated by SimpleBuild (type comma-separated list)
...
``--install-latest-eb-release``                                          |Install latest known version of simplebuild (default: False)
...
``-p, --pretend``                                                        |Does the build/installation in a test directory located in $HOME/simplebuildinstall (default: False)
...
``--silence-deprecation-warnings=SILENCE-DEPRECATION-WARNINGS``          |Silence specified deprecation warnings out of (python2, Lmod6, simpleconfig, toolchain) (type comma-separated list)
...
``--verify-simpleconfig-filenames``                                        |Verify whether filename of specified simpleconfigs matches with contents (default: False)
...
``--try-update-deps``              |Try to update versions of the dependencies of an simpleconfig based on what is available in the robot path (default: False)
